...
int main(int argc, char* argv[]) {
	...
	while(getline(input_file, input_line)) {
		...
		if ((range1low >= range2low && range1high <= range2high) || (range2low >= range1low && range2high <= range1high)) {
			subrange_counter++;
			if (verbose) {
				std::cout << "Subrange counter incremented!" << std::endl;
			}
		}
		else if (verbose) {
			std::cout << "Subrange counter not incremented!" << std::endl;
		}

		if (verbose) std::cout << std::endl;
	}

	std::cout << "There were "<< subrange_counter << 
		" pairs where one range fully contained the other." << std::endl;
