...
int main(int argc, char* argv[]) {
    ...
    while(getline(input_file, crate_row)) {
    	if (crate_row.find("move") != std::string::npos) {
			...
			columns[to_col].insert(columns[to_col].begin(), columns[from_col].begin(), columns[from_col].begin()+num_crates);
			columns[from_col].erase(columns[from_col].begin(), columns[from_col].begin()+num_crates);
